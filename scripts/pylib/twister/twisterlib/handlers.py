...
class BinaryHandler(Handler):
    ...
    def _output_handler(self, proc, harness):
        ...
        with open(self.log, "wt") as log_out_fp:
            ...
            timeout_time = time.time() + self.test_timeout()
            while True:
...
class DeviceHandler(Handler):
    ...
    def _create_serial_connection(self, serial_device, hardware_baud,
        ...
        try:
            ser = serial.Serial(
                ...
                timeout=max(flash_timeout, self.test_timeout())  # the worst case of no serial input
            )
    ...
    def handle(self, harness):
        ...
        if hardware.flash_with_test:
            flash_timeout += self.test_timeout()
...
class QEMUHandler(Handler):
    ...
    def handle(self, harness):
        ...
        self.thread = threading.Thread(name=self.name, target=QEMUHandler._thread,
                                       args=(self, self.test_timeout(), self.build_dir,
