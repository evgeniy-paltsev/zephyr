...
struct k_spinlock _z_pthread_spinlock = {};
...
{
	int rc = 0;
	k_spinlock_key_t key = k_spin_lock(&_z_pthread_spinlock);

	if (m->lock_count == 0U && m->owner == NULL) {
		m->lock_count++;
		m->owner = pthread_self();

		k_spin_unlock(&_z_pthread_spinlock, key);
		return 0;
	...
	rc = z_pend_curr(&_z_pthread_spinlock, key, &m->wait_q, timeout);
	if (rc != 0) {
		rc = ETIMEDOUT;
	}
...
int pthread_mutex_unlock(pthread_mutex_t *m)
{
	...
	if (m->lock_count == 0U) {
		...
		if (thread) {
			...
			z_reschedule(&_z_pthread_spinlock, key);
			return 0;
		}
		m->owner = NULL;
